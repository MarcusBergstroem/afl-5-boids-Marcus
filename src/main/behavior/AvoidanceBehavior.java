...
public class AvoidanceBehavior implements BehaviorStrategy {
    
    // To obstacles der er ca 1 tredjedel inde fra hver side
    private final List<Vector2D> obstacles = List.of(
        new Vector2D(666, 350),
        new Vector2D(333, 350)
    );

    // Avoidance radius
    private final double avoidanceRadius = 150.0; 
    ...
    public Forces calculateForces(Boid boid, List<Boid> neighbors) {
        ...
        Vector2D avoidance = calculateAvoidance(boid, weights);

        // Tilføjer avoidance til separation
        separation = new Vector2D(
            separation.x() + avoidance.x()*2,
            separation.y() + avoidance.y()*2
        );
    ...
    private Vector2D calculateAvoidance(Boid boid, FlockWeights weights) {
        ...
        for (Vector2D obstacle : obstacles) {
            double dx = bPosX - obstacle.x();
            double dy = bPosY - obstacle.y();
